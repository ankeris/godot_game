...
func start_server():
	...
	# if server exists, join it instead
	if (err!=OK):
		join_server()
		return
...
remote func unregister_player(id):
	for child in $"../level_1/players/".get_children():
		if str(id) == child.name:
			child.queue_free()
	players.erase(id)
...
func spawn_player(id):
	...
	if id == get_tree().get_network_unique_id():
		...
		player.player_id = id
		player.control   = true
	# don't add a player for server
	player.set_network_master(id)
	if id != 1:
		$"../level_1/players/".add_child(player)
